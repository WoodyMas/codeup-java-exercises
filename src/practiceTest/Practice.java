...
public class Practice {
    ...
    int[] numberThreeTest = {3, 4, 55, 9, 10};
        System.out.println(Practice.average(numberThreeTest));

    Pet newPet = new Pet();
    Pet otherPet = new Pet("Fido");
    Pet youngPet = new Pet("Wishbone", "Cat");
    Pet myPet = new Pet("Mo", 13, "Poodle mix");

    PetDog pep = new PetDog("Peppy", "Poodle", true);
        System.out.println(pep.getName());
        System.out.println(pep.getType());
        System.out.println(pep.isTrained());

//        for (int number: multiplyAll(5, new ArrayList<>(List.of(5, 10, 15, 25, 30)))){
//            System.out.println(number);
//        }

        System.out.println(multiplyAll(5, new ArrayList<>(List.of(5, 10, 15, 25, 30))));

        System.out.println(multiplyAll(5, new ArrayList<>(List.of(20, 12, 22, 5))));

    PetDog.allSnuggle(new ArrayList<>(List.of()));
    ...
    public static int subtractTen(int integer) {
        return integer - 10;
    };
    ...
    public static double average(int[] arrayOfInt){
        ...
        return (double) acc / arrayOfInt.length;
    }
    ...
    public static ArrayList<Integer> multiplyAll (int multiplier, ArrayList<Integer> numbers){
        ArrayList<Integer> newNumsArrayList = new ArrayList<>();
        for (int number : numbers) {
            newNumsArrayList.add(number * multiplier);
        }
        return newNumsArrayList;
    }
